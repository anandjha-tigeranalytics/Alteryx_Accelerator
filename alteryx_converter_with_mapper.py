...
# Alteryx to Snowflake Function Mapping
alteryx_to_snowflake_mapper = {
    ...
    # New DateTime Mappings
    'DateTimeToString'  : 'TO_CHAR',
    'StringToDateTime'  : 'TO_DATE',
    ...
    "DATETIMEYEAR"      : "YEAR",
    "DATETIMETRIM"      : "MONTH",  # Only when second param is 'month'

    # DateTime Format Specifier Mappings
    "d": "D", "dd": "DD", "day": "Day", "dy": "DY", "EEEE": "Day",
    "M": "M", "MM": "MM", "MMM": "Mon", "MMMM": "MMMM", "Mon": "MONTH", "Month": "MONTH",
    "yy": "YY", "yyyy": "YYYY",
...
def apply_alteryx_to_snowflake(expression):
    """
    Converts Alteryx-style expressions into Snowflake-compatible syntax using the mapper.
    This includes:
    - Function mappings (from alteryx_to_snowflake_mapper)
    - DateTime conversions
    - ISNULL / NOT ISNULL transformations
    - IF-THEN-ELSE to CASE WHEN conversion
    """
    # Apply direct replacements using the mapper
    for alteryx_func, snowflake_func in alteryx_to_snowflake_mapper.items():
        expression = re.sub(rf"\b{alteryx_func}\b", snowflake_func, expression, flags=re.IGNORECASE)

    # Convert `IF-THEN-ELSE-ENDIF` to `CASE WHEN`
...
def sanitize_expression_for_filter_formula_dynamic_rename(expression, field_name=None):
    ...
    expression = re.sub(r"\b(TO_NUMBER)\s*\(\s*([^,]+)(,\s*[^)]+)?\s*\)", fix_to_number, expression, flags=re.IGNORECASE)
...
def sanitize_datetime_format(alteryx_format, is_input=True):
    ...
    Uses the `alteryx_to_snowflake_mapper` for mapping.
    """
    ...
    # Apply Alteryx to Snowflake conversion using the mapper
    snowflake_format = apply_alteryx_to_snowflake(alteryx_format)
    
    # Remove any wildcard characters (*)
    snowflake_format = snowflake_format.replace("*", "")
...
def generate_cte_for_Filter(xml_data, previousToolId, toolId, filterType, prev_tool_fields, toolName, previousToolName):
    ...
    filter_expression = apply_alteryx_to_snowflake(filter_expression)
...
def generate_cte_for_Formula(xml_data, previousToolId, toolId, prev_tool_fields, toolName, previousToolName):
    ...
    for field in formula_fields.findall('FormulaField'):
        ...
        format_ifs_nulls = apply_alteryx_to_snowflake(sql_expression)

        # Apply fix for ISNULL and NOT (!) operators
        # format_nulls = fix_isnull_not_operator(format_ifs)

        formula_expr.append(f"{format_ifs_nulls} AS \"{field_name}\"")
