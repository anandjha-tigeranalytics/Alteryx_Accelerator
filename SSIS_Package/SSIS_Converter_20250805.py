    ...
    def extract_event_handler_task_details(self, package_path):
        ...
        try:
            ...
            for event_handler in root.findall(".//DTS:EventHandler", ns):
                ...
                event_handler_name = ""
                if ref_id and "Package\\" in ref_id:
                    parts = ref_id.split("\\")
                    if len(parts) > 1:
                        event_handler_name = parts[1].split(".")[0]

                # EventHandlerType: Derived from container type (if any tasks inside)
                event_handler_type = ""
                ...
                for executable in event_handler.findall(".//DTS:Executable", ns):
                    ...
                    event_handler_type = task_type  # If only one task inside handler, take this as handler type
                    ...
                    if "insert into" in sql_query.lower():
                        # Get schema or DB name before first dot
                        match = re.search(r'insert\s+into\s+([^\s(]+)', sql_query, re.IGNORECASE)
                        if match:
                            full_name = match.group(1)  # e.g., ods_mode.dbo.ods_error
                            task_connection_name = full_name.split(".")[0]  # ods_mode
